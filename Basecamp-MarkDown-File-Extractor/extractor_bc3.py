...
def sanitize_filename(name):
    ...
    name = re.sub(r'[\\/*?:"<>|]', "", name)
    # Strip leading dots and spaces to prevent path traversal
    name = name.lstrip(". ")
...
def scrape_project(context, project, manifest):
    ...
    for l in links:
        ...
        if "/message_boards/" in href and not tool_links["message_boards"]:
            tool_links["message_boards"] = full_href
        if "/todosets/" in href and not tool_links["todosets"]:
            tool_links["todosets"] = full_href
        if "/vaults/" in href and not tool_links["vaults"]:
            tool_links["vaults"] = full_href
        if "/chats/" in href and not tool_links["chats"]:
            tool_links["chats"] = full_href
    ...
    if tool_links["message_boards"]:
        ...
        page.goto(
            tool_links["message_boards"], wait_until="domcontentloaded", timeout=60000
        )
        ...
        print(
            f"  -> Synced {new_count} new messages (skipped {len(msg_urls) - new_count})"
        )
    ...
    if tool_links["todosets"]:
        ...
        for l in page.query_selector_all("a[href*='/todolists/']"):
            ...
            if (
                href
                and "trash" not in href
                and "/edit" not in href
                and "/new" not in href
            ):
                href = href.split("#")[0]
                full = f"https://3.basecamp.com{href}" if href.startswith("/") else href
        ...
        print(
            f"  -> Synced {new_count} new to-dos (skipped {len(todo_urls) - new_count})"
        )
...
def extract_projects(page):
    ...
    for article in articles:
        ...
        if title and link:
            ...
            if (
                href
                and ("/projects/" in href or "/buckets/" in href)
                and href not in seen_urls
            ):
                full_url = (
                    f"https://3.basecamp.com{href}" if href.startswith("/") else href
                )
